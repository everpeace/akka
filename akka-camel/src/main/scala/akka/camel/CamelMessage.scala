...
class RichMessage(message: CamelMessage, camelContext: CamelContext) {
  /**
   ...
   */
  def withBodyAs[T](implicit m: Manifest[T]): CamelMessage = withBodyAs(m.erasure.asInstanceOf[Class[T]])
   ...
   * Java API
   */
  def withBodyAs[T](clazz: Class[T]): CamelMessage = message.withBody(getBodyAs(clazz))
